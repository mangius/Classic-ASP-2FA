...
var SUPPORTED_ALGS = 4 | 2 | 1;
 ...
 // Define Totp constructor
  function Totp(expiry, length) {
    this.expiry = expiry || 30; // Default expiry
    this.length = length || 6; // Default length
  ...
  Totp.prototype.getOtp = function (secret, now) {
    ...
    var time = this.leftpad(this.dec2hex(Math.floor(epoch / this.expiry)), 16, "0");
  ...
  Hotp.prototype.getOtp = function (key, counter) {
    ...
    var digest = shaObj.getHMAC("HEX");

    var h = this.hexToBytes(digest);
    var offset = h[19] & 0xf;
